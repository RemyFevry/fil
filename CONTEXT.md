...
*Avoid*: framework, wrapper, runtime (in the loose sense).
...
*Avoid*: agent (too generic — here it means the whole tool, not a single model call).
...
*Avoid*: session, execution, instance (too generic).

**Change**:
The unit of work a Run delivers — a feature, fix, or refactor. One Run : one Change.
*Avoid*: ticket, task, story, feature (too narrow — a Change may be a fix or refactor).
...
*Avoid*: workspace, repo (Fil's Project is broader than the git repository).
...
*Avoid*: state (collides with XState's active-state snapshot), step, stage, task.
...
*Avoid*: check, validation (too generic).
...
*Avoid*: result, log (too generic).

**Trigger** *(folded into Gate)*:
