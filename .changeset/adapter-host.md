...
- The deepening (codebase-design: "two adapters means a real seam") lands the
  ...
  `@fil/adapter-host` and owns only its target.

The seam this draws is the host-fs surface; target-specific behaviour stays
in each Adapter. No behaviour change — the lifted helpers are identical to the
duplicates they replace, and the existing installer tests (which drive the
full install path end-to-end) pass unchanged.
